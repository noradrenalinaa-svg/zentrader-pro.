...
<style>
    ...
    .pay-btn { background-color: black; color: white !important; padding: 15px 30px; border-radius: 10px; text-decoration: none; display: block; margin: 20px auto; font-family: 'Orbitron'; font-weight: bold; width: 220px; text-align:center; transition: 0.3s; }
...
if not st.session_state['auth']:
    ...
    with ab_col:
        st.markdown('<div class="about-box"><p style="font-size: 18px;"><b>ZenTrader AI</b> è un sistema di controllo del rischio matematico. Abbiamo creato questo terminale per imporre un <b>Drawdown rigido del 2%</b>, garantendo la longevità del tuo account.</p></div>', unsafe_allow_html=True)

    # SEZIONE PREZZI
    st.markdown('<h2 class="section-title">Piani di Abbonamento</h2>', unsafe_allow_html=True)
    _, p_col, _ = st.columns([1, 1, 1])
    with p_col:
        # Nota: target="_self" forza l'apertura nella stessa scheda
        st.markdown("""
<div class="price-card">
    ...
    <a href="https://www.paypal.com/cgi-bin/webscr?cmd=_xclick&business=noradrenalinaa@gmail.com&item_name=ZenTrader%20AI%20Elite&amount=49.00&currency_code=EUR" target="_self" class="pay-btn">ACQUISTA LICENZA</a>
    <a href="https://t.me/ZenTraderIA" target="_self" class="tg-btn">Parla con noi su Telegram</a>
</div>
""", unsafe_allow_html=True)
...
# --- 6. TERMINALE OPERATIVO (AREA RISERVATA) ---
    ...
    st.info("Benvenuto Luca. Protezione Drawdown attiva al 2%.")
